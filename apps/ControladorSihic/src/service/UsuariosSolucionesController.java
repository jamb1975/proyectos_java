...
public class UsuariosSolucionesController extends AbstractFacade<UsuarioSoluciones>{
 private static final String FIND_TERCEROS_QUERY = 
    ...
    {
      return  EntityManagerGeneric.em.createQuery("select s from Solucion s where s.solucionPadre.id=:pid order by s.solucion")
              .setParameter("pid",1)
              .getResultList();
    }
    ...
    public List<UsuarioSoluciones> getUsuarioSoluciones(String usua,String password)
    {
        try
        {
             ...
             auditoria.setUsuarios(usuario);
             auditoria.setTipo_proceso("0");
             auditoria.setModulo("login");
             auditoria.setDescripcion("Conexión Aplicativo Sihic");
             if(EntityManagerGeneric.et.isActive())
             {
                 EntityManagerGeneric.et.rollback();
             }
