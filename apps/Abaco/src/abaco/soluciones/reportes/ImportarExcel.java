        ...
        Sheet sheet = libro.getSheet(1);
         ...
         for(int i=5;i<751;i++)
         {
          ...
          try{
            ...
            AbacoApp.s_producto.setCodigobarras(codigo.getContents());
            try{
            AbacoApp.s_producto.setPrecio(new BigDecimal(precio.getContents()));
            }catch(Exception e)
            {
                AbacoApp.s_producto.setPrecio(new BigDecimal(0));
            }
         
            AbacoApp.s_proveedores.setNombre(proveedor.getContents());
            AbacoApp.s_proveedores.setNo_ident(codigo.getContents());
            AbacoApp.s_categoria.setM_NombreCat(categoriae.getContents());
            AbacoApp.s_categoria.setCodigo(codigo.getContents());
            
            
             if(hasmap.get(proveedor.getContents().trim().replace(" ",""))==null)
             {      
             
               proveedoresControllerClient.create();
               hasmap.put(proveedor.getContents().trim().replace(" ",""), AbacoApp.s_proveedores);
             }
             else
             {
               AbacoApp.s_proveedores= hasmap.get(proveedor.getContents().trim().replace(" ","")); 
             }
             if(hasmap2.get(categoriae.getContents().trim().replace(" ",""))==null)
             {      
              
               categoriaControllerClient.save();
               hasmap2.put(categoriae.getContents().trim().replace(" ",""), AbacoApp.s_categoria);
             }
             else
             {
               AbacoApp.s_categoria= hasmap2.get(categoriae.getContents().trim().replace(" ","")); 
             }
