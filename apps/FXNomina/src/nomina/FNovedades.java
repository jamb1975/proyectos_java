...
public class FNovedades extends Application {
    ...
    public Parent createContent() throws IOException {
        ...
        util.ValidacionesCampo.ValidarTextField(valor);
        ...
        if (permitirproceso) {
            try {
                set_datos();
                    ...
                    FXNomina.nominaEmpleadosControllerClient.update();
    ...
    private void set_datos() 
    {
        ...
        FXNomina.novedades.setTiponovedad(novedad.getSelectionModel().getSelectedIndex());
        FXNomina.novedades.setTipohoraextra(LoadChoiceBoxGeneral.cb_factorhorasextras.getSelectionModel().getSelectedIndex());
        try
        {
        FXNomina.novedades.setCantidad(Integer.valueOf(cantidad.getText()));
        }catch(Exception e)
        {
             FXNomina.novedades.setCantidad(0);
        }
        try
        {
        FXNomina.novedades.setValor(new BigDecimal(valor.getText()));
        }catch(Exception e)
        {
             FXNomina.novedades.setValor(BigDecimal.ZERO);
        }
        if(jenum.EnumTipoNovedad.HORASEXTRAS1.ordinal()==novedad.getSelectionModel().getSelectedIndex())
        {
          FXNomina.novedades.setValor((FXNomina.nominaEmpleados.calcularvalordiasueldo().divide(BigDecimal.valueOf(FXNomina.nomina.getHorasdiarias()),2,RoundingMode.HALF_UP)).multiply(BigDecimal.valueOf(FXNomina.novedades.getCantidad())).multiply(LoadChoiceBoxGeneral.v_factorhorasextras.get(LoadChoiceBoxGeneral.cb_factorhorasextras.getSelectionModel().getSelectedIndex())));
        ...
        FXNomina.novedades.setObservacion(tipohoraextra);
        }
        else
        {
         FXNomina.novedades.setObservacion(observacion.getText());
        }
         FXNomina.nominaEmpleados.addnovedad(FXNomina.novedades);
    }
...
//         backgroundThread.setDaemon(false);
    ...
    private void nuevoRecord() {
        FXNomina.novedades = null;
        FXNomina.novedades = new Novedades();
        for (Object n : gp_generic.getChildren().toArray()) {
            if (n.getClass() == TextField.class) {
                TextField general = (TextField) n;
                general.setText("");
    ...
    public void crearoeditar() {
        ...
        if (FXNomina.novedades != null) {
            if (FXNomina.novedades.getId() != null) {
             cantidad.setText(String.valueOf(FXNomina.novedades.getCantidad()));
             valor.setText(FXNomina.novedades.getValor().toString());
             novedad.getSelectionModel().select(FXNomina.novedades.getTiponovedad());
             LoadChoiceBoxGeneral.cb_tiposdocumentoscontables.getSelectionModel().select(LoadChoiceBoxGeneral.v_factorhorasextras.indexOf(FXNomina.novedades.getTipohoraextra()));
             observacion.setText(FXNomina.novedades.getObservacion());
            } else {
                nuevoRecord();
            }
        } else {
