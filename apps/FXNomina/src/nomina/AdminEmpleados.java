...
public class AdminEmpleados extends Application {
    ...
    public static void getRecords() throws ParseException {
        try {
            ...
            FXNomina.empleadosControllerClient.getRecords(buscar.getText().trim());
         
            ol_empleados.clear();
            ol_empleados.addAll(FXNomina.li_empleados.toArray());
            System.out.println("size->"+FXNomina.li_empleados.size());
...
//set scene to stage
    ...
    private void checkregistroexistente() throws NoSuchMethodException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
        if ((tv_empleados.getSelectionModel()) != null) {
            FXNomina.empleados = (Empleados) tv_empleados.getSelectionModel().getSelectedItem();
            show();
        }
    }
