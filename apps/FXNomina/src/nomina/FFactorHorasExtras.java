...
public class FFactorHorasExtras extends Application {
    ...
    public void save() throws InterruptedException {
        ...
        if (permitirproceso) {
            try {
                set_datos();
                if (FXNomina.factorHorasExtras.getId() == null) {
                    FXNomina.factorHorasExtrasControllerClient.create();
                } else {
                    FXNomina.factorHorasExtrasControllerClient.update();
                }
    ...
    private void set_datos() {
        ...
        FXNomina.factorHorasExtras.setFactor_dominicalyfestivo(new BigDecimal(dominicalyfestivo.getText().trim()).setScale(2));
        FXNomina.factorHorasExtras.setFactor_dominicalyfestivonocturno(new BigDecimal(dominicalyfestivonocturno.getText().trim()).setScale(2));
        FXNomina.factorHorasExtras.setFactor_extradiurnofestivo(new BigDecimal(extradiurnofestivo.getText().trim()).setScale(2));
        FXNomina.factorHorasExtras.setFactor_extradiurnoordinario(new BigDecimal(extradiurnoordinario.getText().trim()).setScale(2));
        FXNomina.factorHorasExtras.setFactor_extranocturnofestivo(new BigDecimal(extranocturnofestivo.getText().trim()).setScale(2));
        FXNomina.factorHorasExtras.setFactor_extranocturnoordinario(new BigDecimal(extranocturnoordinario.getText().trim()).setScale(2));
        FXNomina.factorHorasExtras.setFactor_recargonocturno(new BigDecimal(recargonocturno.getText().trim()).setScale(2));

    }
  private void getRecord() {
       
       dominicalyfestivo.setText( FXNomina.factorHorasExtras.getFactor_dominicalyfestivo().toString());
       dominicalyfestivonocturno.setText( FXNomina.factorHorasExtras.getFactor_dominicalyfestivonocturno().toString());
       extradiurnofestivo.setText( FXNomina.factorHorasExtras.getFactor_extradiurnofestivo().toString());
       extradiurnoordinario.setText( FXNomina.factorHorasExtras.getFactor_extradiurnoordinario().toString());
       extranocturnofestivo.setText( FXNomina.factorHorasExtras.getFactor_extranocturnofestivo().toString());
       extranocturnoordinario.setText( FXNomina.factorHorasExtras.getFactor_extranocturnoordinario().toString());
       recargonocturno.setText( FXNomina.factorHorasExtras.getFactor_recargonocturno().toString());
...
//         backgroundThread.setDaemon(false);
    ...
    private void nuevoRecord() {
        FXNomina.factorHorasExtras = null;
        FXNomina.factorHorasExtras = new FactorHorasExtras();
        for (Object n : gp_generic.getChildren().toArray()) {
            if (n.getClass() == TextField.class) {
                TextField general = (TextField) n;
                general.setText("");
    ...
    public void crearoeditar() {
        ...
        if (FXNomina.factorHorasExtras != null) {
            if (FXNomina.factorHorasExtras.getId() != null) {
                  getRecord();
            } else {
                nuevoRecord();
