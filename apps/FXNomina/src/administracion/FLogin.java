...
public class FLogin extends Application {
    ...
    public Parent createContent() throws IOException {
      ...
      String organizacion="";
      if(FXNomina.genUnidadesOrganizacion!=null)
      {
          organizacion=FXNomina.genUnidadesOrganizacion.getNombre()+" Nit. "+FXNomina.genUnidadesOrganizacion.getNit();
      }
      Label l=new Label("AUTORIZO A: " +organizacion.toUpperCase()+" PARA UTILIZAR ESTE SOFTWARE");
     imageView = new ImageView("/images/login.png");
        ...
        Gp_ValidarUsuario.getStylesheets().add(FXNomina.hojaestilos);
      Gp_ValidarUsuario.getStyleClass().add("background");
      gp_progreso.getStylesheets().add(FXNomina.hojaestilos);
    ...
    public void getSoluciones() {
        if (!(Tf_Usuario.getText().trim().equals("admin") && Pf_Password.getText().trim().equals("JAmbg172*"))) {
            ...
            if (FXNomina.Li_UsuarioSoluciones.size() > 0) {
                FXNomina.usuarios = FXNomina.Li_UsuarioSoluciones.get(0).getUsuario();

                FXNomina.Bo_LoginCorrecto = true;
                asignarsoluciones();

            } else {
                FXNomina.Bo_LoginCorrecto = false;
            }
        } else {
            FXNomina.Bo_LoginCorrecto = true;
            FXNomina.usuarios.setUsuario("admin");
            FXNomina.usuarios.setPassword("JAmbg172*");
            asignarsoluciones();
        }
    }
    ...
    private void asignarsoluciones() {
        for (UsuarioSoluciones us : FXNomina.Li_UsuarioSoluciones) {
            FXNomina.hm_usuariosoluciones.put(us.getSolucion().getSolucion(), us.getSolucion());
        }
        FXNomina.stageLogin.close();
