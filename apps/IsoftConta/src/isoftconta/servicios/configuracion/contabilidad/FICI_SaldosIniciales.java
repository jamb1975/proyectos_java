...
import isoftconta.servicios.configuracion.contabilidad.FSaldosIniciales;
...
public class FICI_SaldosIniciales extends Application {
    ...
    public Parent createContent() throws IOException, ClassNotFoundException, InstantiationException, IllegalAccessException, ParseException, InterruptedException {
        ...
        FICI_LibroAuxiliar.refrescardatossegunorigen=EnumDocumentoSoporte.SALDOINICIAL19.ordinal();
    ...
    { 
       ...
       Glyph glyph_libromayor=  IsoftConta.fontAwesome.create(FontAwesome.Glyph.LEANPUB.getChar()).sizeFactor(2).color(randomColor).useGradientEffect();
    ...
    public void changed(ObservableValue<? extends Boolean> arg0, Boolean oldPropertyValue, Boolean newPropertyValue)
    {
     DocumentoSoporteController.findpucporcodigo(puc.getText());
        ...
        bu_trasladaralibromayor = new Button("",glyph_libromayor);
        bu_trasladaralibromayor.setTooltip(new Tooltip("Trasladar Saldos Iniciales a Libro Mayor"));
        bu_trasladaralibromayor.setOnAction(e
                -> {
            try {
                   LibroMayorBalancesController.saldosinicales();
            } catch (Exception ex) {
                ex.printStackTrace();
            }
            });
      ...
      hb_tercero.getChildren().addAll(tercero,bu_tercero);
        ...
        toolBar=new ToolBar(bu_save, nuevo, bu_printcomprobante,bu_trasladaralibromayor);
    ...
    private  void set_datos() {
      ...
      EntidadesStatic.es_documentosoporte.setTipodocsoporte(EnumDocumentoSoporte.SALDOINICIAL19.ordinal());
   ...
   {    noconsecutivo.setText(EntidadesStatic.es_documentosoporte.getNocomprobantecerosizquierda());
       ...
       {
           tercero.setText(EntidadesStatic.es_terceros.getNo_ident()+" "+EntidadesStatic.es_terceros.getNombres());
       }
    ...
    public void removeDetalle() throws ParseException, InterruptedException, NoSuchMethodException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
      ...
      int td=0;
            if(((LibroAuxiliar) tv_detalles.getSelectionModel().getSelectedItem()).getConPuc().getCodigo().substring(0, 1).equals("1"))
            {
                td=3;
            }
            else
            {
              if(((LibroAuxiliar) tv_detalles.getSelectionModel().getSelectedItem()).getConPuc().getCodigo().substring(0, 1).equals("2"))
            {
                td=4;
            }  
            }
            LibroAuxiliarController.removerregistrosidsource(td, EntidadesStatic.id_source_mov);
  ...
  private boolean checkregistroexistente() throws NoSuchMethodException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
   ...
   public static void colocardatosproveedor()
   {
       ...
       if(EntidadesStatic.es_terceros!=null)
       {
          concepto.setText(DocumentoSoporteController.getdocumentodoporte(EnumDocumentoSoporte.COMPROBANTECAUSACIONEGRESO14)[0]+ " para pago de Tercero: "+EntidadesStatic.es_terceros.getNombres());
       }
   }
}
