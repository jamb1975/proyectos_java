...
public class FICI_ComprobanteIngresoMasivo extends Application {
   ...
   private GridPane gp_generico;
    ...
    private StageGeneric stagecomprobanteingreso=new StageGeneric("isoftconta.servicios.activos.efectivo.FICI_ComprobanteIngreso", "Comprobante de  Ingreso",true);
    ...
    public void start(Stage stage) throws Exception {
     ...
     public Parent createContent() throws IOException, ClassNotFoundException, InstantiationException, IllegalAccessException, ParseException, InterruptedException {
       ...
       ToolBar toolBar=new ToolBar( bu_ejecutar);
                
                  
        tp_generic = new TitledPane();
        tp_generic.setText("Facturas a Ingresar");
        tp_generic.setCollapsible(false);
        dp_fechaelaboracion = new DatePicker(LocalDate.now());
        dp_fechaelaboracion.setPromptText(" Fecha Elaboración");
  ...
  buscar.setOnAction(e->{
        ...
        nofactura.setCellValueFactory(new Callback<TableColumn.CellDataFeatures<LibroAuxiliar, String>, ObservableValue<String>>() {
            ...
            public ObservableValue<String> call(TableColumn.CellDataFeatures<LibroAuxiliar, String> documentosoporte) {
                ...
                return new SimpleStringProperty(documentosoporte.getValue().getDocumentoSoporte().getNofacturacerosizquierda());

            }
        });
        TableColumn valor = new TableColumn();
        valor.setMinWidth(150);
        valor.setText("Valor Factura");
        valor.setCellValueFactory(new Callback<TableColumn.CellDataFeatures<LibroAuxiliar, String>, ObservableValue<String>>() {
            @Override
            public ObservableValue<String> call(TableColumn.CellDataFeatures<LibroAuxiliar, String> documentosoporte) {

                return new SimpleStringProperty(documentosoporte.getValue().getDebe().toString());
        ...
        TableColumn<LibroAuxiliar, Boolean> cingresar = new TableColumn<>("Ingresar");
        ...
        tv_generic.getColumns().addAll(fechaelab, nofactura,valor,cliente,cingresar);
        ...
        gp_generico.addRow(0, new Label("Fecha Inicial"),new Label("Fecha Final"),new Label("Fecha Elaboración"),new Label("Buscar"),new Label("Centro de Costos"),new Label("Sucursal"),new Label("Buscar N° Comprobante"),new Label("Ingresar"));
        gp_generico.addRow(1,dp_fechadesde,dp_fechahasta, dp_fechaelaboracion,buscar,IsoftConta.cb_centrocostos,IsoftConta.cb_sucursales,tf_nocomprobante, toolBar);
        gp_generico.add(tv_generic, 0, 2, 7, 1);
     ...
     public static void getRecords() throws ParseException{
          ...
          ol_generic.addAll(DocumentoSoporteController.getrecordsdocumentossoporte2(dp_fechadesde.getValue().toString(),dp_fechahasta.getValue().toString(),buscar.getText(),EnumDocumentoSoporte.COMPROBANTECAUSACIONINGRESO15.ordinal()));
...
private void show() throws NoSuchMethodException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, ClassNotFoundException, InstantiationException {
 ...
 {     try
 {
       EntidadesStatic.es_documentosoporte=DocumentoSoporteController.findbynoconsecutivo(Long.valueOf(tf_nocomprobante.getText().trim()));
 ...
 }catch(Exception e)
 {
     ...
     for(int i=0;i<tv_generic.getItems().size();i++)
     {
         if(((LibroAuxiliar)tv_generic.getItems().get(i)).isIngresar())
         {
             if(j==0)
             {
                 inicionofactura=((LibroAuxiliar)tv_generic.getItems().get(i)).getDocumentoSoporte().getNofacturacerosizquierda();
             }
                 finnofactura=((LibroAuxiliar)tv_generic.getItems().get(i)).getDocumentoSoporte().getNofacturacerosizquierda();
         // System.out.println("Factura No."+((DocumentoSoporte)tv_generic.getItems().get(i)).getNofacturacerosizquierda());
             ...
             totalingresos=totalingresos.add(((LibroAuxiliar)tv_generic.getItems().get(i)).getDebe());
         ...
         notificationBuilder.text("Facturas Ingresadas Satisfactoriamente")
 ...
 public void save()  {
    ...
    private  void set_datos() {
      ...
      EntidadesStatic.es_documentosoporte.setConcepto("Ingreso de Facturas desde el N° "+inicionofactura+" Hasta el N° "+finnofactura);
  ...
  public void AddDetalle()  {
           ...
           if (EntidadesStatic.es_documentosoporte.getId() == null) 
           {
            ...
            if (EntidadesStatic.es_documentosoporte.getId() != null) {
                ...
                BigDecimal bdvalor=EntidadesStatic.es_libroauxiliar.getDebe();
               ...
               /* DocumentoSoporteController.findpucporcodigo("11010401");
                ...
                if(EntidadesStatic.es_puc.getPorc_base().compareTo(BigDecimal.ZERO)==1)
                {
                  bdvalor=bdvalor.multiply((EntidadesStatic.es_puc.getPorc_base().multiply(BigDecimal.valueOf(0.01))));
                   ...
                   EntidadesStatic.es_documentosoporte.agregarregistroalibroauxiliar(EntidadesStatic.es_puc, false, EntidadesStatic.es_documentosoporte.getConcepto(), bdvalor, UtilDate.localdatetodate(dp_fechaelaboracion.getValue()),EntidadesStatic.es_terceros,EntidadesStatic.id_source_mov);
                  } 
