...
import sihic.historiasclinicas.consultas.AdminAgnConsultorios;
...
public class FOrganizacion extends Application {
    ...
    private Label la_genCentrosPoblados;
    private ChoiceBox genCentrosPoblados;
    private Vector<GenCentrosPoblados> v_genCentrosPoblados = new Vector<GenCentrosPoblados>();
    ...
    public Parent createContent() throws IOException {
        ...
        stack = new StackPane();
        genCentrosPoblados = new ChoiceBox();
        genCentrosPoblados.setMaxWidth(300);
        ...
        la_genCentrosPoblados = new Label("Centro Poblado:");
        ...
        nivelatencion.getProperties().put("requerido", true);
...
// gridpane
        ...
        gp_generic.add(la_nivelatencion, 0, 3);
        gp_generic.add(nivelatencion, 1, 3);
        gp_generic.add(la_sigla, 0, 4);
        gp_generic.add(sigla, 1, 4);
        gp_generic.add(la_genDptos, 0, 5);
        gp_generic.add(LoadChoiceBoxGeneral.getGen_departamentos_id(), 1, 5);
        gp_generic.add(la_genMunicipios, 0, 6);
        gp_generic.add(genMunicipios, 1, 6);
        ...
        if (permitirproceso) {
            ...
            try {
                ...
                if (genUnidadesOrganizacion.getId() == null) {
                    hclconsultasControllerClient.saveOrganizacion(genUnidadesOrganizacion);
                    L_Message.setText("Registro Almacenado");
                } else {
                    genUnidadesOrganizacion = hclconsultasControllerClient.updateOrganizacion(genUnidadesOrganizacion);
                    L_Message.setText("Registro modificado");
                }
                AdminOrganizacion.getOrganizacion();
                SihicApp.loaddatosorganizacion_toobar();
                Task_Message = () -> {
                    try {
                        SetMessage();
                    } catch (InterruptedException ex) {
    ...
    private void set_datos_oranizacion() {
        ...
        genUnidadesOrganizacion.setNivelAtencion(nivelatencion.getValue().toString());
...
//gp_genpersonas.getChildren().remove(index-1);
    ...
    public void check_exist_organizacion() {
        ...
        nivelatencion.setValue(genUnidadesOrganizacion.getNivelAtencion());
