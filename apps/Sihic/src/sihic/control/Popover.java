...
/**
 * A Popover is a mini-window that pops up and contains some application
 * specific content. It's width is defined by the application, but defaults to a
 * hard-coded pref width. The height will always be between a minimum height
 * (determined by the application, but pre-set with a minimum value) and a
 * maximum height (specified by the application, or based on the height of the
 * scene). The value for the pref height is determined by inspecting the pref
 * height of the current displayed page. At time this value is animated (when
...
public class Popover extends Region implements EventHandler<Event> {
    ...
    /**
     * The visual frame of the popover is defined as an addition region, rather
     * than simply styling the popover itself as one might expect. The reason
     * for this is that our frame is styled via a border image, and it has an
     * inner shadow associated with it, and we want to be able to ensure that
     * the shadow is on top of whatever content is drawn within the popover. In
     * addition, the inner edge of the frame is rounded, and we want the content
     * to slide under it, only to be clipped beneath the frame. So it works best
     * for the frame to be overlaid on top, even though it is not intuitive.
     */
    ...
    protected void layoutChildren() {
        ...
        final int width = 550; //(int)getWidth();
    ...
    public void show(Runnable onHideCallback) {
        if (!isVisible() || fadeAnimation != null) {
            ...
            addEventFilter(MouseEvent.MOUSE_CLICKED, popoverHideHandler);
            addEventFilter(ScrollEvent.ANY, popoverScrollHandler);
    ...
    public void hide() {
        if (isVisible() || fadeAnimation != null) {
            removeEventFilter(MouseEvent.MOUSE_CLICKED, popoverHideHandler);
            removeEventFilter(ScrollEvent.ANY, popoverScrollHandler);
